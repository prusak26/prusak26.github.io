...
(function () {
	...
	var version_modss = '2.7.3', API = Lampa.Utils.protocol() + 'api.lampa.stream/', type = '', jackets = {}, cards, ping_auth, manifest, menu_list = [], vip = false, user_id = '', uid = '48f030ca59bf0f6b3d91', IP, logged = false;
	...
	var Modss = {
      ...
      if (Lampa.Storage.field('mods_tv_butt_ch')) Lampa.Keypad.listener.follow('keydown', function (e) {
    		...
    		if (Lampa.Activity.active().component == 'modss_tv' && Lampa.Player.opened()) {
		...
		online: function (back) {
		  function inf() {
  		  balanser = Lampa.Storage.get('online_last_balanser');
  		  data = Lampa.Storage.cache('online_choice_' + balanser[card.id], 3000, {});
  		  is_continue = card.number_of_seasons && data[card.id] && Lampa.Arrays.getKeys(data[card.id].episodes_view).length;
  		  last_s = is_continue ? ('S'+(data[card.id].season + 1)+' - '+(data[card.id].episodes_view[data[card.id].season == 0 ? 1 : (data[card.id].season +1)])+' '+Lampa.Lang.translate('torrent_serial_episode').toLowerCase()) : '';
  		  title = is_continue && Lampa.Storage.field('online_continued') ? '#{title_online_continue} ' : '#{title_online}';
		  }
			function openOnline(){
        Lampa.Activity.push({
        	url: '',
          ...
          items: [{
            title: Lampa.Lang.translate('title_online_continue') + '? ' + last,
            yes: true
          }, {
		  ...
		  var card = cards, balanser, data, is_continue, last_s, title;
		  inf();
			...
			var button = "<div data-subtitle='modss_v".concat(manifest.version, " (14 Balansers, 7 in vip)' class='full-start__button selector view--modss_online'>"+ico+"<span>"+title+"</span>️</div>");
      var btn = $(Lampa.Lang.translate(button));
			btn.unbind('hover:enter hover.click').on('hover:enter hover.click', function () {
		    inf();
		    if(is_continue && Lampa.Storage.field('online_continued')) shows(last_s);
		    else openOnline();
			});
			
			//Lampa.Activity.active().activity.render().find('.view--modss_online').remove();
			if(back == 'delete') Lampa.Activity.active().activity.render().find('.view--modss_online').remove();
			if(back && back !== 'delete') back.find('span').text(Lampa.Lang.translate(title));
		  if (!back && Lampa.Storage.field('mods_onl')) Lampa.Activity.active().activity.render().find('.view--torrent').before(btn);
		},
    ...
    jack:{					
      ...
      jac_my_to:        {url:'jac.my.to', key:1,lang:'df_lg', interv:'healthy'},
      jacred_cf:        {url:'jacred.cf', key:'',lang:'lg', interv:'healthy'},
	...
	var Filmix = {
		...
		showStatus: function (ch) {
			...
			if (status.login) {
				...
				else info = foto + ' - <b>NO PRO</b> - MAX 2160p</span>';
			}
	...
	var ForkTV = {
		...
		check_forktv: function (itm, ar) {
			...
			this.network["native"](ForkTV.url + '?' + ForkTV.user_dev, function (json) {
			    ...
			    ForkTV.act_forktv_id = json.channels[0].title && json.channels[0].title || '';
    ...
    this.search = function (_object,data) {
      ...
      url = Lampa.Utils.addUrlComponent(url, 'query=' + (itm.imdb_id ? encodeURIComponent(itm.imdb_id) : itm.kinopoisk_id ? encodeURIComponent(itm.kinopoisk_id) : encodeURIComponent(select_title)));
      url = Lampa.Utils.addUrlComponent(url, 'field=' + (itm.imdb_id ? 'imdb_id' : itm.kinopoisk_id ? 'kinopoisk_id' : 'title'));
      network.clear();
    ...
    function getFile(element, max_quality) {
      ...
      if (items && items.length) {
        quality = {};
        var mass = [2160, 1080, 720, 480, 360];
        ...
        var preferably = Lampa.Storage.get('video_quality_default', '2160') + 'p';
  ...
  function filmix(component, _object) {
      ...
      if (wait_similars) this.find(sim[0].id);
		...
		this.find = function (filmix_id) {
			if (!window.FX.is_max_qualitie && !window.FX.auth) {
				Filmix.checkPro(Filmix.token, function (found) {
				    ...
				    if (found.user_data.is_pro || found.user_data.is_pro_plus) window.FX.is_max_qualitie = true;
						if (found.user_data.is_pro) window.FX.max_qualitie = 2160;
		  ...
		  function end_search(filmix_id) {
  			...
  			network.silent(API + 'fm/id/' + filmix_id, function (found) {
  				if (found && Lampa.Arrays.getKeys(found).length && (found.movie.length || Lampa.Arrays.getKeys(found.playlist).length)) {
  				  if (/*window.FX.auth && */!window.FX.is_max_qualitie && found.max) {
  				    window.FX.max_qualitie = found.max;
  				  }
  				  /*if(!window.FX.auth && !window.FX.is_max_qualitie) setTimeout(function () {
    			      Modss.showModal(Lampa.Lang.translate('filmix_auth_onl'), function () {
                Lampa.Modal.close();
                Lampa.Controller.toggle('settings');
  			  ...
  			  token: Filmix.token, 
  			  id:"",  			  fmx: JSON.stringify(window.FX), 
  			  logged:logged
  			});
  		}
		};
      ...
      if (results.playlist && Object.keys(results.playlist).length > 0) {
		...
		function getQuality(link){
		  var qualities = link.match(/\[([\d,]*)\]\.mp4/i);//link.match(/.+\[(.+[\d]),?\].+/i);
  			...
  			for (var keyt in results.movie) {
  				var movie = results.movie[keyt];
					filtred.push({
						...
						link: movie.link,
						qualityes: getQuality(movie.link).quals, 
						quality: results.quality + ' - ' + getQuality(movie.link).max + 'p ',
      ...
      network.clear();
      network.timeout(10000);
      network.silent(API + 'fm/streams/'+id_filmix, function (json) {
			  if (json.file) {
		   		var file = json.file;
		   		var preferably = Lampa.Storage.get('video_quality_default', '2160') + 'p';
  				if (json.quality[preferably]) file = json.quality[preferably];
          element.stream = file;
          element.qualitys = json.quality;
          call(element.stream);
        } else error();
      }, error, {
			  link: encodeURIComponent(element.link),
			  qualityes: JSON.stringify(element.qualityes),
			  season: element.season,
			  episode: element.episode,
			  voice: element.voice_name,
			  id:"",			  token: Filmix.token, 
  			fmx: JSON.stringify(window.FX), 
  		  logged: logged
			});
      ...
      component.draw(items, {
        onEnter: function onEnter(item, html) {
          getStream(item, function (stream) {
            ...
            Lampa.Noty.show(data ? network.errorDecode(data) : Lampa.Lang.translate('online_nolink'));
          });
        },
        onContextMenu: function onContextMenu(item, html, data, call) {
          getStream(item, function (stream) {
    ...
    var embed = prox ? prox + 'https://kinobase.org/' : 'https://kinobase.org/';
		var object = _object;
		var data;
		var select_title = '';
		var select_id = '';
			...
			network["native"](embed + url, function (str) {
				...
				if (MOVIE_ID && IDENTIFIER && PLAYER_CUID) {
					...
					network["native"](embed + data_url, function (user_data) {
						var funk_user_data = new Function(user_data.replace('eval', ' var funk_user_data =')+';return funk_user_data;');
            user_data = funk_user_data().toString();
            var VOD_TIME = user_data.match(/vod_time\s*=\s*"\s*(\d+)\s*"\s*;/); 
            if(VOD_TIME) VOD_TIME = VOD_TIME[1];
            var VOD_HASH = user_data.match(/vod_hash\s*=\s*"\s*([^"]+)\s*"\s*;/); 
            if(VOD_HASH) VOD_HASH = VOD_HASH[1];
	...
	function kinokrad(component, _object) {
		...
		function getFile(element, max_quality) {
			var quality = {},
				first = '';
			var preferably = Lampa.Storage.get('video_quality_default', '2160');
    ...
    this.search = function (_object, sim) {
      if (wait_similars) this.find(sim[0].iframe_src);
		...
		this.searchByKinopoiskj = function (_object, kp_id) {
			var _this = this;
			object = _object;
			select_title = object.search;
			var url = embed;
      ...
      network["native"](url, function (json) {
		...
		function parse(str) {
			str = str.replace(/\n/g, '');
			var find1 = str.match("Playerjs\\({.*?\\bfile:'(.*?)'}\\);");
    ...
    function decode(data) {
      ...
      try {
        ...
        var items = component.parseM3U(str).map(function (item) {
          var link = item.link;
          link = link.replace(/(\.mp4):hls:manifest\.m3u8$/i, '$1');
    ...
    this.find = function () {
        ...
        if (balanser == 'videoapi') {
          ...
          url = Lampa.Utils.addUrlComponent(url, 'api_token=3i40G5TSECmLF77oAqnEgbx61ZWaOYaE');
				...
				if (object.movie.kinopoisk_id) url = Lampa.Utils.addUrlComponent(url, 'kinopoisk_id=' + encodeURIComponent(object.movie.kinopoisk_id));
    ...
    this.similars = function (json) {
      ...
      json.forEach(function (elem) {
        ...
        var item = Lampa.Template.get('modss__online_folder', elem);
    ...
    this.draw = function (items) {
      ...
      this.getEpisodes((object.movie.source == 'pub' || balanser == 'pub') || (object.movie.original_language !== 'ja' || object.movie.number_of_seasons >= choice.seasons) ? items[0].season : 1, function (episodes) {
        ...
        items.forEach(function (element, index) {
          ...
          if (serial && element.episode) image.append('<div class="online-prestige__episode-number-season">S' + (element.season || episode.snumber || episode.season_number || 0) + ':E' + (element.episode || episode.number || episode.episode_number || 0) + '</div>');
          if (serial && !episode) {
            ...
            img.src = object.movie.source == 'filmix' ? object.movie.img : object.movie.source == 'pub' ? (episode.thumbnail || object.movie.background_image) : Lampa.TMDB.image('t/p/w300' + (episode ? episode.still_path : object.movie.backdrop_path));
            images.push(img);
          }
  ...
  function collection(object) {
		...
		this.next = function (page) {
			...
			else url = page.replace(/(\d+)\/$/,object.page+'/');
    ...
    Lampa.Template.add('mods_iptv_list', "<div class=\"iptv-list layer--height\"><div class=\"iptv-list__ico\"><svg height=\"36\" viewBox=\"0 0 38 36\" fill=\"none\" xmlns=\"http://www.w3.org/2000/svg\">\n                    <rect x=\"2\" y=\"8\" width=\"34\" height=\"21\" rx=\"3\" stroke=\"white\" stroke-width=\"3\"/>\n                    <line x1=\"13.0925\" y1=\"2.34874\" x2=\"16.3487\" y2=\"6.90754\" stroke=\"white\" stroke-width=\"3\" stroke-linecap=\"round\"/><line x1=\"1.5\" y1=\"-1.5\" x2=\"9.31665\" y2=\"-1.5\" transform=\"matrix(-0.757816 0.652468 0.652468 0.757816 26.197 2)\" stroke=\"white\" stroke-width=\"3\" stroke-linecap=\"round\"/>\n                    <line x1=\"9.5\" y1=\"34.5\" x2=\"29.5\" y2=\"34.5\" stroke=\"white\" stroke-width=\"3\" stroke-linecap=\"round\"/></svg></div><div class=\"iptv-list__title\">#{iptv_select_playlist}</div><div class=\"iptv-list__items\"></div></div>");
    ...
    manifest = {
      ...
      onContextLauch: function onContextLauch(object) {
        Lampa.Activity.push({
					url: '',
					title: Lampa.Lang.translate('title_online') + " - MODS's v" + version_modss,
					component: 'modss_online',
					search: object.title,
    ...
    if (!Lampa.Lang) {
      ...
      Modss.init();
			...
			Lampa.SettingsApi.addParam({
				...
				field: {
					...
					description: Lampa.Lang.translate('online_balanser') + ' Collaps'
				},
				onRender: function (item) {
					if (Lampa.Storage.field('mods_onl')) item.show(); 
					else item.hide();
			...
			Lampa.SettingsApi.addParam({
				...
				onRender: function (item) {
					if (Lampa.Storage.get('pub_access_token') == Pub.tock) item.hide();
					item.on('hover:enter', function () {
						Pub.refreshTok();
					});
				}
		  ...
		  ['HDRezka', 'Kinobase', 'Collaps'].forEach(function (itm) {
				Lampa.SettingsApi.addParam({
					component: 'mods_proxy',
					param: {
      ...
      Lampa.SettingsApi.addParam({
		        ...
		        $('div[data-children="parser"]').on('hover:enter', function(){
  		        ...
  		        Lampa.Select.render().find('.selectbox-item__title').map(function(i, item){
  		          Modss.check($(item).text().toLowerCase().replace(/\./g,'_'), function(e){
  		            $(item).css('color', e ? '#2aff00ff' : '#d10000');
  		          });
  		        });
    		    });
			...
			/*
      if(!Lampa.SettingsApi.getParam('server')) 
      Lampa.SettingsApi.addParam({
				component: 'server',
				param: {
					name: 'torrserv', 
				...
				field: {
					name: 'Публичные TORRServ Ⓜ️', 			
					description: 'Обновится после выхода из настроек' 
				},
				onChange: function (value) { 	
					if (value == '0') Lampa.Storage.set('torrserver_url_two', '');
					if (value == '1') Lampa.Storage.set('torrserver_url_two', 'ny23.tsarea.us:8880');
					if (value == '2') Lampa.Storage.set('torrserver_url_two', 'trs.my.to:8091');
					if (value == 'skaz') Lampa.Storage.set('torrserver_url_two', '88.210.12.237:8090');
